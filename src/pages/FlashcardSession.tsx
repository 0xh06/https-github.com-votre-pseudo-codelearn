...
const FREE_SESSION_CAP = 8;

export default function FlashcardSession() {
  const { srs, updateSrs, subscriptionPlan } = useStore();
  const isPro = subscriptionPlan === 'pro';
  const [currentIndex, setCurrentIndex] = useState(0);
  const [showAnswer, setShowAnswer] = useState(false);
  const [sessionFinished, setSessionFinished] = useState(false);

  const dueItemsFull = useMemo(
    () =>
      Object.entries(srs)
  ...
  const dueItems = isPro ? dueItemsFull : dueItemsFull.slice(0, Math.min(FREE_SESSION_CAP, dueItemsFull.length));
  ...
  useEffect(() => {
    if (dueItems.length > 0 && currentIndex >= dueItems.length) {
      setCurrentIndex(Math.max(0, dueItems.length - 1));
    }
  }, [dueItems.length, currentIndex]);
    ...
    return (
      <div className="container mx-auto px-4 py-20 text-center max-w-lg">
        ...
        <p className="text-[var(--text-dim)] mb-8">
          {dueItemsFull.length === 0
            ? "Aucune carte à réviser pour l'instant."
            : 'Félicitations, vous avez terminé cette session de révision.'}
        </p>
        <Link to="/flashcards" className="btn btn-primary px-8 py-3 w-full">
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-2xl">
      <Seo title="Session de révision" description="Répétition espacée — concepts algorithmiques." />
      ...
      <div className="flex justify-between items-center mb-8 text-sm text-[var(--text-dim)]">
        <span>
          Question {currentIndex + 1} / {dueItems.length}
        </span>
        <Link to="/flashcards" className="hover:text-[var(--text-bright)]">
          Quitter
        </Link>
      </div>
            ...
            >
              <div className="flex-1 overflow-auto">
                ...
                <p className="font-mono text-sm">
                  {currentAlgo?.timeO} (temps) / {currentAlgo?.spaceO} (espace)
                </p>
              </div>
